	...
	There are internally 3 types of objects:
		...
		Variables are like "x", "L0", "A", etc.
			...
			Warning2: if you use "W" or "Wn" as a variable and you pass "W(x+1)" a LambertW function will be assumed
				rather than "W*(x+1)". If you want you text to be interpreted as multiplication, specify it explicitly
				as shown.
...
//private:
	...
	void  _create_storage_if_no_present(size_t = _formula_main::g_s_initial_capacity);
	void _create_storage_no_check(size_t = _formula_main::g_s_initial_capacity);
...
template <class domain_descriptor>
static void* allocator_wrapper(size_t n)
{
	if (n)
	{
...
template <class domain_descriptor>
static void deallocator_wrapper(void* p, size_t n)
{
	if (p && n)
	{
...
template <class domain_descriptor>
void _formula_main<domain_descriptor>::_create_storage_if_no_present(size_t n)
{
...
template <class domain_descriptor>
void _formula_main<domain_descriptor>::_create_storage_no_check(size_t n)
{
	if (n == 0) n = 1;
